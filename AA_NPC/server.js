...
const obtenerLetraAleatoria = () => {
    const letras = "qweadzxc";
    const numeroAleatorio = Math.floor(Math.random() * 8);
    const letraAleatoria = letras.charAt(numeroAleatorio);
...
const partidaIniciada = () => {
  drawMap(mapaActual);
  console.log("Esperando al siguiente movimiento...");
};
    ...
    rl.question("EC: ", (EC) => {
        rl.question("EF: ", (EF) => {
          ...
          try {
            ...
            console.log(error);
            console.log("Error al enviar la autenticación del NPC");
          }
        });
      });
...
console.log("Pulse enter para introducir los efectos del NPC al clima...");
// rl.write('\n');
rl.on("line", (input) => {
  if (!UsuarioLogeado) 
    unirNPC();
  else { 
    setTimeout(() => {}, (Math.floor(Math.random() * 3000) + 1));
    const letra = obtenerLetraAleatoria();
    manejarEntradaPartida(letra);}
});
